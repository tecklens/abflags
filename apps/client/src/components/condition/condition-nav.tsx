...
export default function ConditionNav({operator, onChangeOperator, addRule, addGroup}: ConditionNavProps) {
  return (
    <div className={'flex justify-between items-center'}>
      <Tabs value={operator} onValueChange={(v: any) => onChangeOperator(v)} className={'flex flex-col space-y-1 text-xs h-7'}>
      ...
      <div className={'flex gap-2'}>
        <Button size={'sm'} type={'button'} className={'py-0 h-6 text-green-500'} onClick={() => addRule()}>
          <IconPlus size={14}/>
          <span>Add rule</span>
        </Button>
