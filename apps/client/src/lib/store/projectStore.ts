...
import {SettingState} from "@client/lib/store/settingStore";
...
export const useProject = create(
  persist<OrgState>((set, getState) => ({
      ...
      projects: [],
      starProjects: [],
      starProject: (projectId: string) => {
        set({
          starProjects: [...(getState().starProjects ?? []), projectId]
        })
      },
      unStarProject: (projectId: string) => {
        set({
          starProjects: (getState().starProjects ?? []).filter(e => e !== projectId)
        })
      },
      fetchMembers: async () => {
