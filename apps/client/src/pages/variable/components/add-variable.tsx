import {Dialog, DialogContent, DialogFooter, DialogTitle, DialogTrigger} from "@client/components/ui/dialog";
...
export default function AddVariable() {
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    ProjectRepository.createVariable(data).then((resp: AxiosResponse) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger>
        <Button size={'sm'}><IconPlus size={16}/>Add variable</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogTitle>
          Add variable
        ...
        <Form {...form}>
          <form id={'add-variable'} onSubmit={form.handleSubmit(onSubmit)} className={'grid gap-3'}>
            <FormField
        ...
        <DialogFooter>
          <Button form={'add-variable'} type={'submit'}>Create</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
